...
class DecodedInstruction {
	 ...
	 * 
	 * @param instruction
	 *            : A string of the form <TYPE> <ADDR>, where TYPE can be {READ,
	 *            WRITE} and 0 <= ADDR < 1024
	 * @param pid
...
public class CoherenceProtocol {
	 ...
	 * 
	 * @param processors
	 *            : Array containing the processors in the system
	 */
	...
	boolean needToAccessBus(DecodedInstruction instruction) {
		...
		if (instruction.type == "READ"
				&& (state == ProtocolState.S || state == ProtocolState.E || state == ProtocolState.M)) {
			return false;
		}

		// Writing when in E or M state will not generate a bus access
		else if (instruction.type == "WRITE"
				&& (state == ProtocolState.E || state == ProtocolState.M)) {
			return false;
		}
	...
	public void simulateMESIProtocol() {
		...
		Random r = new Random();
		DecodedInstruction instruction_p0 = new DecodedInstruction(
...
/*
	 ...
	 *            : Id of processor issuing the instruction
	 */
	void executeMOESIInstruction(DecodedInstruction instruction, int pid) {
